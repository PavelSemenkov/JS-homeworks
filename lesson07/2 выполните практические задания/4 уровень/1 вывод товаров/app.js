...
document.querySelectorAll('button').forEach(function (button) {
    ...
    //из атрибута data-type у кнопки, по которой кликнули.
        button.addEventListener('click', function (event){
            ...
            showCategory(target.dataset.type);
        })
});
/**
...
function showCategory(category) {
    for (let i in products[category]) {
        getProductMarkup(products[category][i].name, products[category][i].imageUrl, products[category][i].price, products[category][i].id)
    }
}
 ...
 * в верху этого файла.
 */
function getProductMarkup(name, imageUrl, price, id) {
    let product = `
        <div class="product">
            <div>${name}</div>
